...
- **subtitle_segments** (optional): Split this scene's `script_line` into 1-4 logical subtitle chunks. Break at natural pauses: sentence ends, commas, or breath points. Each chunk is 2-8 words. The chunks must rejoin into the exact `script_line` (same words, same order, just split). Write the chunks in the same language as the `script_line` (Persian chunks for Persian lines, English chunks for English lines). Boundaries ONLY — NEVER include timestamps. You decide WHERE subtitle chunks break, not WHEN they appear; the application derives all timing from the generated audio.
