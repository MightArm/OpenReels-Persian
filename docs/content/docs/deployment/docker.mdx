...
- **pnpm** (pinned version, installed via `npm install -g`) for package management
...
# Install pnpm, install deps, install Chrome for Remotion
...
# Copy source and build frontend with the workspace-local vite
COPY . .
